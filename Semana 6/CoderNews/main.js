...
// function buscador (titulo, noticiaBuscador){
//     return titulo.includes(noticiaBuscador.toLowerCase())
// }

// const noticiaABuscar = prompt("¿Que noticia esta buscando?")

// const noticiaEncontrada = noticias.find(el=>{
//     return buscador(el.contenido.toLowerCase(), noticiaABuscar)
// })
...
const noticiasFiltradas = noticias.filter(el => {
    return el.autor.includes("Andrés López")
})
...
const estaEsaNoticia = noticias.some(el => {
    return el.autor.includes("Nacho")
})


// if(estaEsaNoticia){
//     alert("Efectivamente, hay una noticia de el autor que busca")
// }else{
//     alert("Ese no trabaja aca...")
// }
...
const todosLosTitulos = noticias.map(el =>{ 
    return el.titulo
})
...
const todosLosTitulosEnMayuscula = todosLosTitulos.map(el => el.toUpperCase())

console.log(todosLosTitulosEnMayuscula)

// Crea una función que utilice reduce() para obtener la cantidad total de noticias.

const total = noticias.reduce((acu)=>{
    return acu * 2
},1) 
